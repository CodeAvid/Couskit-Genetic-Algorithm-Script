        ...
        for j in range(max_generations):
            ...
            if j % 200 == 0:
                ...
                requests.get(api_url, params={
                             "current_progress": j // 2, "total_progress": max_generations - 1, "timetableId": timetable_id})

        # print('Run', i + 1, 'cost', cost_function(chromosome), 'chromosome', chromosome)
        requests.get(api_url, params={
                     "current_progress": max_generations // 2, "total_progress": max_generations - 1, "timetableId": timetable_id})
    ...
    for j in range(3 * max_generations):
        ...
        if j % 200 == 0:
            ...
            requests.get(api_url, params={"current_progress": (
                j // 6) + (max_generations // 2), "total_progress": max_generations - 1, "timetableId": timetable_id})
